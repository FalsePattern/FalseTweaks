...
public class QuadCenterComputer implements CenterComputer {
    public static final QuadCenterComputer INSTANCE = new QuadCenterComputer();
    @Override
    public float getCenter(float x, float y, float z, int[] vertexData, int i, int vertexSize) {
        ...
        val dx = Float.intBitsToFloat(vertexData[i + vertexSize * 2]) - x;
        val dy = Float.intBitsToFloat(vertexData[i + vertexSize * 2 + 1]) - y;
        val dz = Float.intBitsToFloat(vertexData[i + vertexSize * 2 + 2]) - z;

        val xAvg = (ax + bx + cx + dx) / 4f;
        val yAvg = (ay + by + cy + dy) / 4f;
        val zAvg = (az + bz + cz + dz) / 4f;
